...
export const templateUsage = mysqlTable("template_usage", {
  id: int("id").autoincrement().primaryKey(),
  templateId: varchar("template_id", { length: 100 }).notNull(), // e.g., "financial_analysis", "executive_assistant"
  userId: int("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  agentConfigId: int("agent_config_id").references(() => agentConfigs.id, { onDelete: "set null" }), // null if agent was deleted
  clonedAt: timestamp("cloned_at").defaultNow().notNull(),
  completedAt: timestamp("completed_at"), // when agent was successfully created
  abandoned: int("abandoned").default(0).notNull(), // 1 if user left wizard without completing
});
...
export const templateReviews = mysqlTable("template_reviews", {
  ...
  review: text("review"),
  helpful: int("helpful").default(0).notNull(), // count of users who found this helpful
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
});
...
export const templateMetadata = mysqlTable("template_metadata", {
  id: int("id").autoincrement().primaryKey(),
  templateId: varchar("template_id", { length: 100 }).notNull().unique(),
  featured: int("featured").default(0).notNull(), // 1 if featured on homepage
  verified: int("verified").default(0).notNull(), // 1 if verified by platform team
  totalClones: int("total_clones").default(0).notNull(),
  totalCompletions: int("total_completions").default(0).notNull(),
  avgRating: int("avg_rating").default(0).notNull(), // stored as integer (rating * 100) for precision
...
export type TemplateMetadata = typeof templateMetadata.$inferSelect;
export type InsertTemplateMetadata = typeof templateMetadata.$inferInsert;
