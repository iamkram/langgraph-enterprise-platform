...
/**
 * Execute an agent by ID for scheduled execution
 */
export async function executeAgent(params: {
  ...
  // Parse worker agents and tools
  let workerAgents: any[] = [];
  let tools: any[] = [];
  
  try {
    workerAgents = agent.workerAgents ? JSON.parse(agent.workerAgents as string) : [];
  } catch (error) {
    console.error('Error parsing worker agents:', error);
  }
  ...
  // Execute agent using simulation
  const config = {
    name: agent.name,
    description: agent.description || '',
    agentType: agent.agentType as "supervisor" | "single" | "sequential",
    model: agent.modelName,
    workers: workerAgents.map((w: any) => ({
      name: w.name || w,
      description: w.description || '',
      systemPrompt: w.systemPrompt || agent.systemPrompt || 'You are a helpful AI assistant.',
    })),
    tools,
    security: {
      enablePiiDetection: agent.securityEnabled === 1,
      enableGuardrails: agent.securityEnabled === 1,
      enableCheckpointing: agent.checkpointingEnabled === 1,
    },
  };

  const inputString = typeof parsedInput === 'string' ? parsedInput : JSON.stringify(parsedInput);
  const result = await simulateAgentExecution(config, inputString);
